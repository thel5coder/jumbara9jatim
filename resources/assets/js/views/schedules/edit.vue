<template>
    <div>
        <page-header :page-section="'Ubah Jadwal'" :page-title="'Jadwal'" :active-section="'Ubah'"
                     :breadcrumb-items="breadcrumbItems" :heading-items="headingItems"></page-header>
        <div class="page-container">
            <div class="page-content">
                <div class="content-wrapper">
                    <div class="row">
                        <div class="col-md-12" v-if="schedule">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-2">
                                                    <input type="text" class="form-control startPickTime"
                                                           placeholder="jam kegiatan" v-model="schedule.scheduleTime">
                                                </div>
                                            </div>
                                            <div class="form-group">
...
<script>
    ...
    export default {
        ...
        computed:{
            ...
            ...mapGetters('SchedulesStore',{schedules:'getSchedules',schedule:'getSchedule'})
        },
        mounted(){
            let app = this;
            this.$store.dispatch('TasksStore/browse',{url:'task'})
            if(this.schedules == null){
                this.$store.dispatch('SchedulesStore/read',{id:this.$route.params.uuid,index:null})
            }else{
                let dataIndex = this.schedules.findIndex(data => data.uuid === this.$route.params.uuid)
                this.$store.dispatch('SchedulesStore/read',{id:this.$route.params.uuid,index:dataIndex})
            }
            ...
            $('.startPickTime').pickatime({
                ...
                onClose(){
                    if(app.schedule.scheduleDate != null){
                        ...
                        app.schedule.scheduleTimeInteger = scheduleTimeInteger
                        app.schedule.scheduleTime = this.get()
                    }
                }
            });
        ...
        methods:{
            ...mapActions('SchedulesStore',{editSchedule:'edit'}),
            validateForm(){
                ...
                validator.validateAll().then((result)=>{
                    if(result){
                        this.editSchedule({input:this.schedule}).then((result)=>{
